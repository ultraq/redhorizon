...
import nz.net.ultraq.redhorizon.converter.Png2ShpConverter
import nz.net.ultraq.redhorizon.converter.ShpFileWriterOptions
			...
			if (!destFile.exists()) {
				sourceFile.withInputStream { inputStream ->
					destFile.withOutputStream { outputStream ->
						new Png2ShpConverter(inputStream).convert(outputStream, new ShpFileWriterOptions(width, height, numImages))
					}
				}
				return 0
			}
