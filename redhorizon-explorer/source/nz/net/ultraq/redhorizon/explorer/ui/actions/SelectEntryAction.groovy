...
/**
 * Command for selecting an entry in the entry list and previewing it.
 *
...
class SelectEntryAction {
	...
	void select() {
		...
		if (entry instanceof MixEntry) {
			// TODO: Should there be some sort of parent relationship so that we don't
			//       need to reach into the UIController to get the last directory?
			if (entry.name() == '..') {
