...
import static nz.net.ultraq.redhorizon.classic.filetypes.cps.CpsFile.*
 ...
 */
@InheritConstructors
class CpsFileWriter extends FileWriter<ImageFile> {

	@Override
	void write(ImageFile imageFile, Map options = [:]) {

		def output = new NativeDataOutputStream(outputStream)
		def lcw = new LCW()
		def palette = imageFile instanceof InternalPalette ? imageFile.palette : null

		// Encode image
		def encodedImage = lcw.encode(imageFile.imageData, ByteBuffer.allocateNative(imageFile.imageData.capacity()))
